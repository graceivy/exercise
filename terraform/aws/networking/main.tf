...
resource "aws_vpc" "mtc_vpc" {
  ...
  # When the vpc is destroyed, the gw has nowhere to go. So need to create a new VPC before destroying it.
  lifecycle {
...
resource "aws_subnet" "mtc_public_subnet" {
  ...
  availability_zone       = ["us-east-1a", "us-east-1b"][count.index] #random_shuffle.az_list.result[count.index] changed it as "us-east-1e" does not supported t3.micro
